...
char *main_func(char *str, Prototype pr) {
  char *res, *dot_place;
  int n = strlen(str), sign_of_num, len_drob_part;
  if (*str == '-') {
    sign_of_num = 1;
    str = str + 1;
  } else {
  ...
  if (n >= pr.width_number) { // ok
    ...
    if (n >= pr.prec_number) {                   // ok
      ...
      if (pr.spec == 'd') {
        ...
        if (pr.minus_flag) {
          res = add_char_right(buf_res, pr.width_number - strlen(buf_res) - 1, (int)strlen(buf_res) + 1, ' ');
        } else {
          res = add_char(buf_res, pr.width_number - strlen(buf_res) - 1,
                         strlen(buf_res) + 1, ' ');
        }
      }
    } else if (n < pr.prec_number) {// точность и ширина больше длины - херачим пробелы и нули перед числом
